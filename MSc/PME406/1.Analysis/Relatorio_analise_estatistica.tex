...
As métricas de desempenho resumem o ajuste completo com interação. Embora
...
ou efeitos adicionais de carga e interação. A Estimativa é o valor
...
\midrule
Interação carga-DAC & Intercepto & 46,6181 & 0,0328 & 1423,28 &
\(<0{,}001\) & [46,5538; 46,6825] \\
\midrule
Interação carga-DAC & Nível de DAC & -28,1177 & 0,0174 & -1614,54 &
\(<0{,}001\) & [-28,1519; -28,0835] \\
\midrule
Interação carga-DAC & Carga de 2 kOhm & -1,0877 & 0,0862 & -12,62 &
\(<0{,}001\) & [-1,2569; -0,9184] \\
\midrule
Interação carga-DAC & Carga de 4,7 kOhm & -3,6991 & 0,2542 & -14,55 &
\(<0{,}001\) & [-4,1984; -3,1998] \\
\midrule
Interação carga-DAC & Interação DAC-carga de 2 kOhm & 0,5210 & 0,0494 & 10,55 &
\(<0{,}001\) & [0,4240; 0,6180] \\
\midrule
Interação carga-DAC & Interação DAC-carga de 4,7 kOhm & 1,9616 & 0,1529 & 12,83 &
\(<0{,}001\) & [1,6613; 2,2619] \\
\midrule
...
reta média para todas as cargas. O ajuste com interação é o modelo
...
global pode ser visto como uma versão simplificada do modelo com interação,
obtida quando os termos adicionais de carga e interação são considerados
nulos. Assim, a hipótese nula do teste é:
\[
H_0: \text{os termos adicionais de carga e interação não reduzem o erro de forma relevante}.
\]
A hipótese alternativa é:
\[
H_1: \text{os termos adicionais reduzem o erro residual de forma estatisticamente significativa}.
\]
...
modelos aninhados, essa razão pode ser escrita como:
\[
F =
...
interação. A diferença entre esses valores é:
\[
85{,}63 - 47{,}25 = 38{,}38\ \mathrm{mA}^2,
\]
que representa a redução da soma dos quadrados residual ao adicionar os
termos de carga e interação. O modelo completo usa quatro parâmetros
adicionais em relação ao modelo global, por isso a coluna GL da segunda
linha mostra o valor 4. Substituindo os valores na expressão do teste:
\[
F =
...
evidência estatística de que considerar a carga e sua interação com o
comando de DAC melhora o ajuste em relação ao modelo global. Assim, para
...
mínimos quadrados. Entretanto, o teste de Shapiro-Wilk apresenta
p-valor inferior a 0,001, indicando que a hipótese de normalidade dos
resíduos é questionável.
...
Há evidência de heterocedasticidade; portanto, a hipótese de variância
constante dos resíduos é questionável.
...
\bottomrule\noalign{}
\endlastfoot
Durbin-Watson & 0.068879 & \(<0{,}001\) &
Evidência de autocorrelação temporal dos resíduos \\
\end{longtable}
}
\tablecaptionbelow{Teste de autocorrelação temporal dos resíduos}

Há evidência de autocorrelação temporal; assim, os p-valores clássicos
da regressão podem estar otimistas.

\begin{figure}
\centering
\includegraphics[width=0.74\linewidth,height=0.34\textheight,keepaspectratio]{figures/11_residual_acf.png}
...
\textbf{Amostras}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Pontos influentes}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Pontos influentes (\%)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Limite de Cook}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Maior distância de Cook}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Maior resíduo studentizado abs.}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
O gráfico de distância de Cook foi mantido por sintetizar visualmente os
pontos com maior influência no ajuste. Os demais critérios aparecem na
tabela, pois são mais úteis como valores de referência do que como
figuras isoladas.

\begin{figure}
\centering
\includegraphics[width=0.74\linewidth,height=0.32\textheight,keepaspectratio]{figures/12_cook_distance_by_index.png}
