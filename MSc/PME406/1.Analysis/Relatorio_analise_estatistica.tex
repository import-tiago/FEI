...
\item
  comparar o modelo global com o ajuste que considera a interação entre
  carga e comando de DAC;
\item
...
\toprule
Carga & Faixa medida (V) & Faixa comum (V) & Inclinação ref. (mA/V) & Limite de inclinação (mA/V) & Pontos retidos & Pontos removidos & Removidos (\%) \\
\midrule
1k & -43.80 a 46.33 & -46.33 a 46.33 & 28.153 & 14.077 & 322 & 3 & 0.923 \\
2k & -46.22 a 49.56 & -46.33 a 46.33 & 27.808 & 13.904 & 168 & 157 & 48.308 \\
4k7 & -47.62 a 47.78 & -46.33 a 46.33 & 26.605 & 13.303 & 76 & 249 & 76.615 \\
\bottomrule
\end{tabular}
}%
\end{center}
...
comparação posterior com o ajuste que inclui a interação entre carga e
comando de DAC verifica justamente se adicionar dependência explícita da
carga melhora de forma relevante essa representação.
...
interação entre carga e comando de DAC, que testa formalmente se a carga
altera a relação entre DAC e corrente.
...
\section{Interação entre carga e comando de
...
O objetivo deste ajuste não é substituir a análise anterior por uma
técnica desconectada do conteúdo da disciplina, mas complementar o modelo
global. No modelo global, uma única reta é usada para representar todas
...
cargas. Ao incluir a interação entre carga e comando de DAC, verifica-se
se a relação entre tensão de DAC e corrente é a mesma para todas as
cargas ou se muda levemente conforme a carga conectada.

Em termos estatísticos, uma interação indica que o efeito de uma variável
explicativa depende do nível de outra variável. Neste relatório, isso
significa perguntar se a variação da corrente causada por uma mudança na
tensão de DAC é igual para todas as cargas. Se não houver interação
relevante, as retas associadas às cargas tendem a ser paralelas. Se
houver interação relevante, a inclinação da reta também muda com a carga.
Assim, a carga pode alterar tanto o deslocamento vertical da reta quanto
sua inclinação:
\[
...
corrente ao comando de DAC. Se os termos de interação forem relevantes,
isso indica que as curvas das cargas não são paralelas: além de terem
interceptos diferentes, também possuem inclinações ligeiramente distintas.

A equação completa estimada para o ajuste com interação é:
\[
\begin{aligned}
I\,[\mathrm{mA}] =\;& 46{,}6181
...
A partir dos coeficientes do ajuste com interação, as retas finais por
carga são:
\[
\begin{aligned}
...
Essas equações deixam explícito que o ajuste com interação não produz
uma única reta igual à do modelo global. Ele produz uma reta de
...
\caption{Retas estimadas pelo ajuste com interação entre carga e comando de DAC}
\end{figure}

\FloatBarrier

A figura mostra a diferença prática entre o modelo global e o ajuste com
interação. A linha preta tracejada representa a reta global única,
...
A tabela a seguir quantifica esse desempenho. Ela possui apenas uma linha
porque resume uma única formulação estatística, embora essa formulação
contenha termos separados para as cargas de 2 kOhm e 4,7 kOhm em relação
à referência de 1 kOhm.
...
\bottomrule\noalign{}
\endlastfoot
Interação carga-DAC & 0.999813 & 0.999811 & 0.290484
& 566 & 0.223411 & 0.288941 & 0.74369 \\
\end{longtable}
}
\tablecaptionbelow{Desempenho do ajuste com interação entre carga e comando de DAC}

Ao incluir a interação entre carga e comando de DAC, o ajuste reduz o
...
estatística dos dados, o ajuste com interação representa melhor o
...
adicional for aceitável, pois o uso da interação exige conhecer ou
...
Interação carga-DAC & 0.05 & Rejeita \(H_0\) &
Intercepto & 46.618138 & 0.032754 & 1423.279796 & 0 & 46.553802 &
46.682473 \\
Interação carga-DAC & 0.05 & Rejeita \(H_0\) & Nível de DAC
& -28.117716 & 0.017415 & -1614.538039 & 0 & -28.151923 & -28.083509 \\
Interação carga-DAC & 0.05 & Rejeita \(H_0\) & Carga de 2 kOhm &
-1.087684 & 0.086165 & -12.623199 & 0 & -1.256931 & -0.918437 \\
Interação carga-DAC & 0.05 & Rejeita \(H_0\) & Carga de 4,7 kOhm &
...
A comparação formal entre o modelo global e o ajuste com interação entre
carga e comando de DAC foi feita por ANOVA/teste F para modelos
aninhados.
...
& & \\
Interação carga-DAC & 0.05 & 2 & 560 &
47.253472 & 4 & 38.37632 & 113.699262 & 0 \\
\end{longtable}
}
\tablecaptionbelow{Comparação por ANOVA entre o modelo global e o ajuste com interação}

Incluir a interação entre carga e comando de DAC melhora
significativamente o ajuste em relação ao modelo global. A queda da soma
dos quadrados residual de 85,63 para 47,25 mA\textsuperscript{2} mostra
redução relevante da variabilidade não explicada, e o teste F elevado,
com p-valor abaixo de 0,05, indica que os quatro termos adicionais
associados à carga e à interação não são apenas complexidade supérflua.
Assim, para fins de descrição estatística, a formulação com interação é
superior; para controle simples em malha aberta, o modelo global ainda
pode ser útil se o erro adicional for aceitável.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Banda de confiança da corrente em função do DAC}]{figures/07_current_vs_dac_confidence_band.png}}
...
Ha evidencia de heterocedasticidade; a hipotese de variancia constante e
questionavel.
...
\tablecaptionbelow{Teste de autocorrelação temporal dos resíduos}

Ha evidencia de autocorrelacao temporal; p-valores classicos da
regressao podem estar otimistas.
...
\tablecaptionbelow{Resumo de pontos influentes e critérios de influência}
