...
Linear regression was then performed independently for each load and globally using all three loads within the common valid linear region. For the global model, the three independent experimental current measurements, corresponding to the 1 k$\Omega$, 2 k$\Omega$, and 4.7 k$\Omega$ loads, were restricted to the linear operating region. The resulting samples were reorganized into a single dataset in which each point was represented by a pair $(V_{\mathrm{DAC}}, I_{\mathrm{out}})$, with the load label retained only as auxiliary information. All points from the three loads were then combined and used simultaneously in a single ordinary least-squares linear regression. The objective of the global fit was to identify a single embedded DAC estimation model valid across the evaluated loads inside the hardware-supported compliance interval. Residuals were analyzed to assess systematic deviations from the global model, and current spread between loads was computed to evaluate load consistency.
...
The preferred embedded model is a single global linear model identified using all three loads within the valid linear region. Figure~\ref{fig:global_model} shows the experimental curves and the resulting global linear model.
...
\caption{Absolute current spread between loads.}
