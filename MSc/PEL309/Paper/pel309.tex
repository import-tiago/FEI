...
\begin{abstract}
Functional Electrical Stimulation (FES) systems require predictable control of stimulation current to obtain repeatable output conditions and support predictable embedded control. This paper characterizes the relationship between digital-to-analog converter (DAC) control voltage and output current in a Howland-current-source output stage related to the STIMGRASP architecture. A dedicated firmware was used only to generate a DAC ramp for characterization, and output current was measured under 1 k$\Omega$, 2 k$\Omega$, and 4.7 k$\Omega$ resistive loads. The complete dataset was used to characterize the experimental operating limits, whereas compliance-limited samples were excluded before linearity and regression analyses. In the selected valid load-voltage interval, Pearson and Spearman correlations were close to -1 for all loads, confirming highly linear and monotonic behavior. A single global linear model was identified using all three loads for embedded feedforward DAC estimation.
\end{abstract}
